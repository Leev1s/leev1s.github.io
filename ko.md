---
title: Lev1s（한국어）
permalink: /ko/
---

# _**LI Junxing**_

_李俊星_ _Jasen_

[العربية](/ar/) · [EN](/) · [Español](/es/) · [فارسی](/fa/) · [Français](/fr/) · [हिन्दी](/hi/) · [日本語](/ja/) · **한국어** · [Русский](/ru/) · [中文](/zh/)

> 이 사이트에 오신 것을 환영합니다! 이 페이지에는 저에 대한 자세한 정보가 담겨 있습니다.\
> 개인 이메일: lev1s at duck dot com

## 🙋 프로필

강한 연구 엔지니어링 및 시스템 구현 능력을 보유하여, 문제 정의에서 방안 설계, 실험 검증, 엔지니어링 납품까지 완전한 파이프라인을 독립적으로 추진할 수 있습니다. 학부 단계에서는 수학 모델링 대회와 연구 훈련을 통해 탄탄한 모델링 분석·추상 표현·팀워크 능력을 쌓았으며, 대학원 단계에서는 암호화폐 퀀트 연구와 이미지 분할 후처리 알고리즘 분야에서 교차 실천을 수행하여 퀀트 연구 프레임워크 재구축·시계열 예측 파이프라인 구축, 그리고 주류 분할 워크플로우에서의 [RankSEG](https://github.com/rankseg/rankseg) 적응·실험 재현·환경 패키징을 완성했습니다. 오픈소스 협력·기술 문서 작성·이중 언어 소통에 능숙하며, Agent 워크플로우·자동화 툴체인·클라우드 배포 관련 실무 경험을 보유하고 있습니다.

## 🎓 학력

- **홍콩 시립대학교** *(2025년 9월 – 2026년 10월)*  
  *생물통계학 이학 석사*  

- **칭다오 기술대학교** *(2020년 9월 – 2024년 6월)*  
  *수학 및 응용수학 이학 학사*  


## 💼 인턴십 및 연구 경험

### **홍콩 중문대학교（CUHK）**
*파트타임 연구 보조원 | 2026년 2월 – 현재*  
- Supervised by [Prof. Ben Dai](https://www.bendai.org/).
- [RankSEG](https://github.com/rankseg/rankseg) 이미지 분할 후처리 알고리즘의 연구 엔지니어링화 및 구현을 추진하여, 소스 코드 읽기와 실험 재현, 주류 분할 프레임워크 적응, 추론 후처리 통합, 평가 스크립트 개발부터 클러스터 환경 패키징까지 전체 파이프라인을 담당하여 알고리즘을 연구 프로토타입에서 재현 가능하고 배포 가능한 엔지니어링 형태로 발전시켰습니다.  
- 주류 시맨틱 세그멘테이션 워크플로우에서 [RankSEG](https://github.com/rankseg/rankseg) 통합과 확장을 추진하였으며, mmsegmentation 및 PaddleSeg 프레임워크 적응에 참여하고, Transformers 포크에서 post-processing helper를 설계하여 모델 출력 확률 텐서가 Dice/IoU 지향 후처리에 직접 입력될 수 있도록 하여 재학습 없이 분할 예측 결과를 최적화할 수 있도록 지원했습니다.  
- 추론 후처리 및 평가 파이프라인 재구축에 참여하여, Dice/IoU 메트릭을 중심으로 예측 워크플로우를 최적화하고 로깅, 평가 스크립트와 결과 추적을 보강하여 실험 비교 가능성, 디버깅 효율 및 재현 안정성을 향상시켰습니다.  
- CUHK HPC 클러스터 환경에서 Singularity + Slurm 기반의 재현 가능한 실험 방안을 구축하여 이미지 패키징, 의존성 관리, 작업 템플릿 제출 및 멀티 GPU 환경 적응을 완성하고, 분산된 실험 명령을 통합 템플릿으로 정리하여 A100, V100, RTX Pro 6000 환경에서 배치 실험과 평가를 지원했습니다.  
- 커뮤니티 사용자와 잠재적 협력자를 위한 프로젝트 납품 파이프라인을 강화하여 퀵스타트 가이드, 통합 튜토리얼, README, 인터랙티브 데모와 플레이그라운드를 보강하고, 클라우드 시연과 CI/CD 파이프라인 구축을 추진하여 신규 사용자의 온보딩 장벽을 낮추고 프로젝트 외부 노출 효율을 향상시켰습니다.

### **저장 밍처 자산관리 유한회사**
*양적 아키텍처 인턴（CityUHK Bios Coop）| 2025년 9월 – 2026년 4월*  
*암호화폐 양적 거래 및 전략 개발*  
- BTC, ETH 및 유동성 Top40 자산을 중심으로 일별 빈도 암호화폐 거래를 위한 시그널 연구와 프레임워크 구축을 수행하며, 온체인 데이터 정리, 특징 구성, 팩터 전처리, 예측 모델링, 시계열 백테스트부터 시그널 생성까지 전체 R&D 파이프라인을 담당했습니다.  
- Random Forest 및 XGBoost 등 트리 모델을 사용하여 활성 팩터 발굴과 시그널 강화를 수행하고, 정상성 검정, 상관관계 검정 및 팩터 분류 전처리 전략을 통해 입력 특징 품질을 향상시키며, 다양한 반응 변수 구성 방법과 횡단면 시그널의 주 시그널 기여도를 탐색했습니다.  
- 초기 R Markdown 기반 실험 스크립트를 모듈형 Python-R-SQL 연구 프레임워크로 재구축하는 것을 주도하여, 코드 디렉토리, 모듈 경계, 함수 인터페이스, 변수 명명, 데이터 구조 및 YAML 설정 메커니즘을 처음부터 설계하여 재사용 가능하고 확장 가능한 양적 연구 및 백테스트 파이프라인을 형성했습니다.  
- 금융 시계열의 고노이즈, 고공선성 문제를 해결하기 위해 MAVE 및 오토인코더 방법을 적용하여 특징 차원 축소와 표현 학습을 수행하고, 엄격한 시계열 교차 검증과 하이퍼파라미터 탐색을 결합하여 정보 누출 위험을 제어하고 모델 평가의 견고성을 향상시켰습니다.  
- 8년간의 역사 데이터와 1년 롤링 백테스트 윈도우를 사용하여 7/14/28일 다중 주기 방향 예측 검증을 실시하고, log return 부호를 핵심 방향 레이블로 분류와 연속 예측 이중 경로 평가 체계를 구축하여 방향 예측 정확도 약 **65%**를 달성했습니다.  
- 다중 자산 병렬 훈련, 예측 및 백테스트를 지원하는 연구에서 생산까지의 시그널 납품 파이프라인을 구축하여, 통합 YAML 설정으로 구동되며 결과를 JSON/API 형식으로 반환하여 SQL 데이터베이스에 저장하고 후속 프론트엔드 표시와 실거래 통합을 지원했습니다.

## 🔬 프로젝트 경험

### **홍콩 자키 클럽 배당률 및 베팅 행동 통계 모델링**
*프로젝트 발기인 | 2025년 10월 – 2025년 12월*  
*시티유 과목 프로젝트*  
- HKJC 역사적 경마 데이터에 대한 대규모 데이터 스크래핑, 정리 및 탐색적 분석을 수행하여 경마 결과, 배당률 및 베팅 유형을 포괄하는 구조화된 분석 데이터셋을 구축했습니다.  
- 통계적 추론과 가설 검정을 적용하여 경마 결과 분포와 잠재적 영향 요인을 체계적으로 분석하고, 샘플 균형과 모델링 실현 가능성을 검증했습니다.  
- 역사적 경마 데이터를 사용하여 **플레이스** 및 **플레이스 Q** 베팅 시나리오에 대한 통계 학습 모델을 구축하고, 특징 엔지니어링, 모델 훈련 및 결과 평가를 완성했습니다.  
- 사틴 경마장 샘플과 소규모 검증에서 높은 예측 정확도를 달성하여 후속 베팅 전략 분석과 모델 최적화를 지원했습니다.  

### **불포화 토양의 전단 강도에 관한 실험 연구**
*핵심 팀원 | 중국 국가자연과학기금 지원*  
[📄 학술 논문（Rock and Soil Mechanics）](https://doi.org/10.16285/j.rsm.2022.2005)  
*Grant No.: 42307236, 12172187, 12072170*  
- 고정밀 **MATLAB 모델** 개발（정확도 > **98%**）  
- 20만 개 이상의 데이터 포인트를 처리하는 **C 코드** 최적화 → **3배 속도 향상**  
- **Python / Origin**을 사용한 시각화 제작  
- **2건의 국가 특허** 공동 출원  

### **다중 규칙 프랙탈 생성 연구**
*졸업 논문*  
- **Taichi**를 사용한 GPU 가속 프랙탈 모델 구축, **100배 속도 향상** 달성  
- 프랙탈 실시간 렌더링 및 상호작용 구현  
- 기존 프랙탈 생성 시스템의 개선 사항 도출  

### **COVID-19 역학 모델링**
*MathorCup 2022*  
- **미디어 및 루머 역학**을 통합한 **$SIERR-T$ 모델** 제안  
- **500만 개의 Weibo 댓글**에 대한 **NLP 감성 분석**  
- 딥러닝 방법으로 루머 식별 및 확산 추세 예측을 수행해 실험 데이터셋에서 우수한 예측 성능을 달성.  
- [OxCGRT 데이터](https://www.bsg.ox.ac.uk/research/research-projects/covid-19-government-response-tracker)를 활용한 정책 효과 분석  


## 📚 논문 및 특허

- **[J.1]** NIU Geng, ZHU Xiao-feng, **LI Jun-xing**, LÜ Meng-yuan, AN Li-qi, CHEN Zi-han. (2022). [**Experimental study on shear strength of unsaturated soil over a wide suction range and its prediction**](https://doi.org/10.16285/j.rsm.2022.2005). *Rock and Soil Mechanics*, No. 12, pp. 1-11. DOI: 10.16285/j.rsm.2022.2005. (IF=3.721, EI, CA, JST, CSCD, Peking University Core Journal)  
- **[P.1]** NIU Geng, **LI Jun-xing**, ZHU Xiao-feng, TAN Yong-ming, MIAO Yu-song, ZHAO Shi-jun, WU Di. (2024). [**Triaxial Apparatus for Unsaturated Soil Based on Dialysis Method Suction Control**](https://patents.google.com/patent/CN117368002A). Chinese Patent, Patent No. CN117368002A. Application No. CN202311298912.7, Application Date: 2023-10-09, Publication Date: 2024-01-09.  
- **[P.2]** NIU Geng, KONG Liang, ZHU Xiao-feng, **LI Jun-xing**, AN Li-qi, LÜ Meng-yuan, CHEN Zi-han. (2022). [**Direct Shear Apparatus for Unsaturated Soil with Erosion Monitoring and Control System**](https://patents.google.com/patent/CN115452613A). Chinese Patent, Patent No. CN115452613A. Application No. CN202211220042.7, Application Date: 2022-10-08, Publication Date: 2022-12-09.


## 🏅 수상 내역

- 🥈 **중국 대학생 수학 경시대회(CMC) 전국 2등상** *(2023년 1월)*
- 🥈 **전국 대학생 수학 모델링 경시대회(CUMCM) 산둥성 2등상** *(2022년 11월)*
- 🥈 **산둥성 대학생 수학 경시대회 산둥성 2등상** *(2022년 11월)*
- 🥉 **MathorCup 수학 모델링 챌린지 전국 3등상** *(2022년 5월)*
- 🥈 **산둥성 대학생 물리 경시대회 산둥성 2등상** *(2021년 11월)*

## 🛠 기술 스택

![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![R](https://img.shields.io/badge/-R-276DC3?style=flat-square&logo=r&logoColor=white)
![MATLAB](https://img.shields.io/badge/-MATLAB-FF4B00?style=flat-square&logo=mathworks&logoColor=white)
![LaTeX](https://img.shields.io/badge/-LaTeX-008080?style=flat-square&logo=latex&logoColor=white)
![SQL](https://img.shields.io/badge/-SQL-E38C00?style=flat-square&logo=mysql&logoColor=white)
![Linux](https://img.shields.io/badge/-Linux-333333?style=flat-square&logo=linux&logoColor=white)
![Docker](https://img.shields.io/badge/-Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![CI/CD](https://img.shields.io/badge/-CI%2FCD-2EA043?style=flat-square&logo=githubactions&logoColor=white)
![Cloudflare](https://img.shields.io/badge/-Cloudflare-F38020?style=flat-square&logo=cloudflare&logoColor=white)
![Shell](https://img.shields.io/badge/-Shell-4EAA25?style=flat-square&logo=gnubash&logoColor=white)
![C](https://img.shields.io/badge/-C-A8B9CC?style=flat-square&logo=c&logoColor=black)
![Git](https://img.shields.io/badge/-Git-F05032?style=flat-square&logo=git&logoColor=white)
![MySQL](https://img.shields.io/badge/-MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![Redis](https://img.shields.io/badge/-Redis-DC382D?style=flat-square&logo=redis&logoColor=white)
![Slurm](https://img.shields.io/badge/-Slurm-009BDE?style=flat-square&logoColor=white)
![Singularity](https://img.shields.io/badge/-Singularity-1D3557?style=flat-square&logoColor=white)
![Jupyter](https://img.shields.io/badge/-Jupyter-F37626?style=flat-square&logo=jupyter&logoColor=white)
![VS Code](https://img.shields.io/badge/-VS%20Code-007ACC?style=flat-square&logo=visualstudiocode&logoColor=white)



## 🌏 언어 능력

- 중국어（표준어）*（모국어）*  
- 영어 *（B2 – 학술·비즈니스 커뮤니케이션）*  


## 🎯 관심사

🏞 하이킹 · 🏋️‍♂️ 피트니스 · 🏍 오토바이 · 📷 사진 · 💻 코딩  

---
*최종 업데이트: 2026년 4월*
