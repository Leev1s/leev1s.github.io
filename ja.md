---
title: Lev1s（日本語）
permalink: /ja/
---

# _**LI Junxing**_

_李俊星_ _Jasen_

[EN](/) · [中文](/zh/) · [العربية](/ar/) · [Français](/fr/) · [Русский](/ru/) · [Español](/es/) · **日本語** · [한국어](/ko/)

> このサイトへようこそ！このページには私についての詳細が記載されています。\
> 個人メール：lev1s at duck dot com

## 🎓 学歴

- **香港城市大学** *(2025年9月 – 2026年10月)*  
  *生物統計学 理学修士*  

- **青島理工大学** *(2020年9月 – 2024年6月)*  
  *数学および応用数学 理学学士*  


## 💼 インターンシップ・研究経験

### **香港中文大学（CUHK）**
*パートタイム研究アシスタント | 2026年2月 – 現在*  
- **NeurIPS** および **JMLR** で発表された最先端の画像分割フレームワーク **[RankSEG](https://github.com/rankseg/rankseg)** の開発に貢献。  
- RankSEG の **PaddleSeg** フレームワークへの統合・最適化を担当し、推論後処理とログ機構を改善。  
- 大規模モデルの再現可能なトレーニングと評価のために **Docker 化された環境** を構成・最適化。

### **浙江明策資産管理有限公司**
*量的アーキテクチャインターン（CityUHK Bios Coop）| 2025年9月 – 現在*  
*暗号通貨量的取引・戦略開発*  
- **十分次元削減（SDR）** 技術を統合した多段階深層学習フレームワークを開発し、暗号通貨時系列予測に適用。**MAVE**、**OPG**、**KSIR** を活用して非線形方向を効果的に抽出。  
- 大規模バックテスト実験を最適化するためのスケーラブルな **ネスト並列処理メカニズム** を設計。資産次元と時間軸次元にわたってタスクを並列化する動的リソーススケジューリング戦略を策定し、**CPU 使用率を最大化**し、大規模モデルトレーニングの **計算負荷を大幅に削減**。

## 🔬 プロジェクト経験

### **非飽和土のせん断強度に関する実験的研究**
*コアチームメンバー | 国家自然科学基金助成*  
[📄 学術論文（岩土力学）](https://doi.org/10.16285/j.rsm.2022.2005)  
- 高精度 **MATLAB モデル**（精度 > **98%**）を開発  
- 20万点以上のデータを処理する **C コード** を最適化し、**3倍の高速化** を達成  
- **Python / Origin** による可視化を作成  
- **2件の国家特許** を共同出願  

### **マルチルールフラクタル生成の研究**
*卒業論文*  
- **Taichi** を使用した GPU 加速フラクタルモデルを構築し、**100倍の高速化** を達成  
- フラクタルのリアルタイムレンダリングとインタラクションを実装  
- 既存のフラクタル生成システムに対する改善点を特定  

### **COVID-19 の疫学モデリング**
*MathorCup 2022*  
- **メディア・デマ動態** を統合した **$SIER_1R_2-T$ モデル** を提案  
- **500万件の Weibo コメント** に対する **NLP 感情分析**  
- パネルデータ予測のための **深層学習モデル**  
- [OxCGRT データ](https://www.bsg.ox.ac.uk/research/research-projects/covid-19-government-response-tracker) を使用して政策効果を分析  


## 📚 論文・特許

- **[J.1]** [非飽和土のせん断強度に関する実験的研究...](https://doi.org/10.16285/j.rsm.2022.2005) – *Rock and Soil Mechanics* (IF=3.721)  
- **[P.1]** [透析法による非飽和土三軸試験装置](https://patents.google.com/patent/CN117368002A) – 中国特許 CN117368002A  
- **[P.2]** [侵食モニタリング付き直接せん断試験装置](https://patents.google.com/patent/CN115452613A) – 中国特許 CN115452613A  


## 🏅 受賞歴

- 🥈 **全国大学生数学コンテスト（CMC）**
- 🥈 **山東省大学生数学コンテスト**
- 🥈 **全国大学生数学モデリングコンテスト（CUMCM）**
- 🥉 **MathorCup 数学モデリングチャレンジ**
- 🥈 **山東省大学生物理コンテスト**
- 🎖️ **文化・スポーツ活動奨学金**



## 🛠 スキル

![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![R](https://img.shields.io/badge/-R-276DC3?style=flat-square&logo=r&logoColor=white)
![MATLAB](https://img.shields.io/badge/-MATLAB-FF4B00?style=flat-square&logo=mathworks&logoColor=white)
![LaTeX](https://img.shields.io/badge/-LaTeX-008080?style=flat-square&logo=latex&logoColor=white)
![SQL](https://img.shields.io/badge/-SQL-E38C00?style=flat-square&logo=mysql&logoColor=white)
![Linux](https://img.shields.io/badge/-Linux-333333?style=flat-square&logo=linux&logoColor=white)
![Docker](https://img.shields.io/badge/-Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![CI/CD](https://img.shields.io/badge/-CI%2FCD-2EA043?style=flat-square&logo=githubactions&logoColor=white)
![Cloudflare](https://img.shields.io/badge/-Cloudflare-F38020?style=flat-square&logo=cloudflare&logoColor=white)



## 🌏 言語能力

- 中国語（普通話）*（母語）*  
- 英語 *（B2 – 学術・ビジネスコミュニケーション）*  


## 🎯 趣味・関心

🏞 ハイキング · 🏋️‍♂️ フィットネス · 🏍 バイク · 📷 写真 · 💻 コーディング  

---
*最終更新：2026年2月*
