...
我的画像明显偏向黑色与蓝色：以行动配合分析。

- 黑色体现为驱动力、主动性，以及把事情推进完成的倾向。
- 蓝色带来精确、好奇，以及理解系统实际运行方式的偏好。
- 红色体现出我对压力、直接表达和高强度工作的部分接受度。
- 白色仍然存在但居于次要位置：我重视能够改善执行的结构。
- 绿色最不突出，意味着我并不天然优先关注情绪氛围或共识营造。

### 核心模式

当面对困难问题、真实约束，并拥有足够空间进行清晰思考时，我通常能发挥最好。相比写出一个有说服力的描述，我更愿意构建一个经得起实践检验的系统。

在团队中，我通常通过解决问题、建立结构和推动执行来贡献价值。我未必总是以热情或高曝光度开始，但会通过让事情真正运转、减少歧义，并把困难任务推进到终点来表达投入。

### 参考

- [SoulTrace 测评结果](https://soultrace.app/en/results/48093f7c-7980-4f83-97a2-e5b8df3a103f)
...
- **需求拆解与交付：**围绕重复性强、依赖经验的工作流，参与 AI 产品支持、内部解决方案、数据自动化和业务运营相关工作。
- **Agent/Skill 设计：**将较为宽泛的业务需求拆解为结构化输入、规则、MVP、反馈闭环和验收标准。
- **内部推广：**支持 Agent 启用、用户培训、SOP 文档和可复用工作流资产建设，并在适当边界内处理业务信息。
- **业务自动化：**针对数据处理、知识检索、报告和演示文稿流程构建与调整实用自动化方案，兼顾视觉质量、可编辑性与复用性。

### **香港中文大学（The Chinese University of Hong Kong, CUHK）**
*兼职研究助理 | 2026年2月 – 至今*  
- **指导教师：**[Prof. Ben Dai](https://www.bendai.org/)。
- **研究工程化：**围绕 [RankSEG](https://github.com/rankseg/rankseg) 图像分割后处理算法开展研究工程化与部署，覆盖实验复现、框架适配、推理集成、评估和集群环境封装。
- **框架适配：**推进 RankSEG 在 mmsegmentation、PaddleSeg、Hugging Face Transformers 以及 SAM 系列模型路径中的使用；设计无需重新训练即可支持 Dice/IoU 后处理的接口。
- **评估与复现：**构建可复现的评估、日志和结果追踪流程，提升实验可比性、排障效率和复现稳定性。
- **HPC 环境：**在 CUHK HPC 上封装 Singularity + Slurm 环境，并准备可复用的多 GPU 实验作业模板。
- **开源贡献：**通过已合并的 [Transformers PR #19](https://github.com/rankseg/rankseg/pull/19)、[SAM 集成 PR #20](https://github.com/rankseg/rankseg/pull/20)、[集成文档 PR #22](https://github.com/rankseg/rankseg/pull/22) 以及 [MONAI PR #2](https://github.com/rankseg/MONAI/pull/2)，参与 RankSEG 生态的公开协作。

### **浙江明策资产管理有限公司（Zhejiang Mingce Asset Management Co., Ltd.）**
*量化架构实习生 | 2025年9月 – 2026年4月*
*量化研究工程与数字资产数据基础设施*
- **数据平台：**搭建量化研究数据平台，覆盖约 8 年历史数据和流动性排名 Top 40 的数字资产，整合 OHLCV 数据、链上指标和衍生因子。
- **列式架构：**管理约 1.7 亿条分钟级市场记录、3,000 个候选特征和约 20 GB Parquet 数据；将架构调整为 Parquet + S3 兼容对象存储 + DuckDB/Polars，并使用 MySQL 管理元数据。
- **查询与更新优化：**按资产、频率、年份和日期分区，加入增量采集、校验和备份；典型研究查询从 1–3 分钟缩短至 5–15 秒，日常更新控制在 10 分钟内。
- **研究 pipeline：**将分散的 Python/R 脚本重构为可配置的 Python–R–SQL 流水线，支持多资产训练、推理、滚动验证、回测和可复现实验。
- **模型验证：**使用 LightGBM、XGBoost、MAVE 和 VAE 开展 7/14/28 天预测与特征表示；某一阶段性滚动样本外结果约为 65% 方向准确率，不应解读为普遍适用的表现。
- **信号交付：**构建标准化的信号交付与模型管理层，通过 API/JSON 接口返回预测结果、因子暴露和策略权重。
...
*香港城市大学课程项目*  
- 整理、清洗历史赛事、赔率和投注类型数据，并使用统计推断与统计学习分析位置和位置 Q 场景。
- 这是一个小规模的城市大学课程项目，结果不应被解读为稳定的投注能力或实盘收益。

### **宽广吸力范围非饱和土剪切强度试验研究及其预测**
*核心团队成员 | 国家自然科学基金相关项目*
[📄 论文（岩土力学）](https://doi.org/10.16285/j.rsm.2022.2005)  
- 在 **MATLAB** 中实现非饱和土剪切强度预测与拟合，模型拟合准确率超过 **98%**；在 **C** 中重写核心函数，支持 200,000+ 个数据点并使计算效率提升约 **300%**。
- 使用 **Origin / Python** 完成分析与可视化，并参与形成两项设备相关专利成果。

### **多规则分形生成研究**
*优秀毕业论文*  
- 构建复杂动力系统驱动的多规则分形生成模型，并实现分形图形的实时渲染与交互式展示。
- 基于 **Taichi（Python）** 实现 GPU 加速计算，相较原始实现获得约 **100 倍**性能提升。  

### **基于时变微分方程动力学的 COVID-19 流行病学建模**
*MathorCup 2022*  
- 构建纳入媒体传播与谣言扩散机制的 **SIERR-T 动力学模型**，并对微博评论开展情感分析与谣言识别。
- 使用 [OxCGRT 数据](https://www.bsg.ox.ac.uk/research/research-projects/covid-19-government-response-tracker) 分析政策影响。
