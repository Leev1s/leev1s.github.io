---
title: Lev1s（中文）
permalink: /zh/
---

# _**李俊星**_

_LI Junxing_ _Jasen_

[EN](/) · **中文** · [العربية](/ar/) · [Français](/fr/) · [Русский](/ru/) · [Español](/es/) · [日本語](/ja/) · [한국어](/ko/)

> 欢迎来到这个页面！这里包含更多关于我的信息。\
> 个人邮箱：lev1s at duck dot com

## 🎓 教育经历

- **香港城市大学** *(2025年9月 – 2026年10月)*  
  *生物统计学理学硕士*  

- **青岛理工大学** *(2020年9月 – 2024年6月)*  
  *数学与应用数学理学学士*  

## 💼 实习与科研经历

### **香港中文大学（CUHK）**
*兼职研究助理 | 2026年2月 – 至今*  
- 参与 **[RankSEG](https://github.com/rankseg/rankseg)** 项目开发（发表于 **NeurIPS** 与 **JMLR** 的图像分割框架）。  
- 负责 RankSEG 与 **PaddleSeg** 的集成与优化，改进推理后处理和日志流程。  
- 配置并优化可复现的 **Docker** 训练与评估环境，用于大规模模型实验。

### **浙江明策资产管理有限公司**
*量化架构实习生（CityUHK Bios Coop）| 2025年9月 – 至今*  
*加密货币量化交易与策略开发*  
- 基于 **充分降维（SDR）** 思路，构建多阶段深度学习时序预测框架，结合 **MAVE**、**OPG** 与 **KSIR** 提取非线性有效方向。  
- 设计可扩展的嵌套并行回测机制，构建按资产维度与时间维度的动态资源调度策略，显著提升 CPU 利用率并降低大规模训练计算开销。  

## 🔬 项目经历

### **非饱和土抗剪强度试验研究**
*国家自然科学基金项目核心成员*  
[📄 论文（岩土力学）](https://doi.org/10.16285/j.rsm.2022.2005)  
- 开发高精度 **MATLAB** 模型（准确率 > **98%**）  
- 优化 **C 代码** 处理 20 万+ 数据点，实现 **3× 加速**  
- 使用 **Python / Origin** 进行可视化分析  
- 共同申请 **两项国家专利**  

### **多规则分形生成研究**
*毕业论文*  
- 使用 **Taichi** 构建 GPU 加速分形模型，实现 **100× 加速**  
- 实现分形图形实时渲染与交互  
- 提出对现有分形系统的改进方向  

### **COVID-19 流行病学建模**
*MathorCup 2022*  
- 提出融合媒体传播与谣言机制的 **$SIER_1R_2-T$ 模型**  
- 对 **500万条微博评论** 开展 **NLP 情感分析**  
- 构建用于面板数据预测的 **深度学习模型**  
- 使用 [OxCGRT 数据](https://www.bsg.ox.ac.uk/research/research-projects/covid-19-government-response-tracker) 分析政策效果  

## 📚 论文与专利

- **[J.1]** [Experimental study on shear strength of unsaturated soil...](https://doi.org/10.16285/j.rsm.2022.2005) – *Rock and Soil Mechanics* (IF=3.721)  
- **[P.1]** [Triaxial Apparatus for Unsaturated Soil Based on Dialysis Method](https://patents.google.com/patent/CN117368002A) – 中国专利 CN117368002A  
- **[P.2]** [Direct Shear Apparatus with Erosion Monitoring](https://patents.google.com/patent/CN115452613A) – 中国专利 CN115452613A  

## 🏅 荣誉奖项

- 🥈 **中国大学生数学竞赛（CMC）**
- 🥈 **山东省大学生数学竞赛**
- 🥈 **全国大学生数学建模竞赛（CUMCM）**
- 🥉 **MathorCup 数学建模挑战赛**
- 🥈 **山东省大学生物理竞赛**
- 🎖️ **文体活动奖学金**

## 🛠 技能

![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![R](https://img.shields.io/badge/-R-276DC3?style=flat-square&logo=r&logoColor=white)
![MATLAB](https://img.shields.io/badge/-MATLAB-FF4B00?style=flat-square&logo=mathworks&logoColor=white)
![LaTeX](https://img.shields.io/badge/-LaTeX-008080?style=flat-square&logo=latex&logoColor=white)
![SQL](https://img.shields.io/badge/-SQL-E38C00?style=flat-square&logo=mysql&logoColor=white)
![Linux](https://img.shields.io/badge/-Linux-333333?style=flat-square&logo=linux&logoColor=white)
![Docker](https://img.shields.io/badge/-Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![CI/CD](https://img.shields.io/badge/-CI%2FCD-2EA043?style=flat-square&logo=githubactions&logoColor=white)
![Cloudflare](https://img.shields.io/badge/-Cloudflare-F38020?style=flat-square&logo=cloudflare&logoColor=white)

## 🌏 语言能力

- 中文普通话（母语）  
- 英语（B2，学术与职场沟通）  

## 🎯 兴趣爱好

🏞 徒步 · 🏋️‍♂️ 健身 · 🏍 摩托车 · 📷 摄影 · 💻 编程  

---
*最后更新：2026年2月*
